...
func (service *TaxService) CalculateTax(tax request.Tax, deduction model.Deduction) response.TaxSummary {
	if tax.TotalIncome < 150000.99 {
		...
		amount := decimal.NewFromFloat((tax.TotalIncome - 150000 - deduction.Personal) * 0.1)
		return response.TaxSummary{Tax: amount.InexactFloat64()}
	} else if tax.TotalIncome >= 500001 && tax.TotalIncome <= 1000000.99 {
		amount := decimal.NewFromFloat((tax.TotalIncome - 500000 - deduction.Personal) * 0.15)
		return response.TaxSummary{Tax: amount.InexactFloat64()}
	} else if tax.TotalIncome >= 1000001 && tax.TotalIncome <= 2000000.99 {
		amount := decimal.NewFromFloat((tax.TotalIncome - 150000 - deduction.Personal) * 0.2)
		return response.TaxSummary{Tax: amount.InexactFloat64()}
	} else {
		amount := decimal.NewFromFloat((tax.TotalIncome - 2000000 - deduction.Personal) * 0.35)
		return response.TaxSummary{Tax: amount.InexactFloat64()}
	}
}
